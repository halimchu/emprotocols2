...
let incrementCPRcovid
let incrementEPIcovid
export {incrementCPRcovid, incrementEPIcovid}
...
class TimerDashboardCovid extends React.Component {
  constructor () {
    super () 
      this.state = {
    ...
    return (dispatch) => {
                  ...
                  const min = () => {
                      let currentMoment = moment() 
                        ...
                        if (minuteCPR >= 60) { //60
                                minuteCPR = 60
                                clearInterval(incrementCPRcovid)
                        } else {
                                // foreground
                  ...
                  clearInterval(incrementCPRcovid)
                  incrementCPRcovid = setInterval( () => dispatch({ 
  ...
  startTimerEPI = () => {
    ...
    return (dispatch) => {
                  ...
                  const min = () => {
                        ...
                        if (minuteEPI >= 60) {
                                minuteEPI = 60
                                clearInterval(incrementEPIcovid)
                        } else {
                                // foreground
                                if (this.state.appState === 'active') {
                                            if (secondEPI === 59) {
                  ...
                  clearInterval(incrementEPIcovid)
                  incrementEPIcovid = setInterval( () => dispatch({ 
...
const mapStateToProps = (state) => {
  return {
    timerTextColorCPR: state.timerReducerCPRCovid.timerTextColor,
    minuteCPR: state.timerReducerCPRCovid.minute,
    secondCPR: state.timerReducerCPRCovid.second,
    buttonStatusNumberCPR: state.timerReducerCPRCovid.buttonStatusNumberCPR,
    resetAllCPR: state.timerReducerCPRCovid.resetAllCPR,


    timerTextColorEPI: state.timerReducerEPICovid.timerTextColor,
    minuteEPI: state.timerReducerEPICovid.minute,
    secondEPI: state.timerReducerEPICovid.second,
    buttonStatusNumberEPI: state.timerReducerEPICovid.buttonStatusNumberEPI,
    resetAllEPI: state.timerReducerEPICovid.resetAllEPI,

    
    CPR: state.buttonsReducer.CPRCovid,
    epi: state.buttonsReducer.epiCovid,
    shock: state.buttonsReducer.shockCovid,
  }
}
...
export default connect(mapStateToProps, mapDispatchToProps)(TimerDashboardCovid)
