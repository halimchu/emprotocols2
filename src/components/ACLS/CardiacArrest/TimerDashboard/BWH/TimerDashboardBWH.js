...
let incrementCPR_BWH
let incrementEPI_BWH
export {incrementCPR_BWH, incrementEPI_BWH}
      ...
      minuteCPR = 0
                            ...
                            const min = () => {
                                let currentMoment = moment() 
                                  ...
                                  if (minuteCPR >= 60) { //60
                                          minuteCPR = 60
                                          clearInterval(incrementCPR_BWH)
                                  } else {
                                          // foreground
                          ...
                          clearInterval(incrementCPR_BWH)
                          incrementCPR_BWH = setInterval( () => dispatch({ 
  ...
  startTimerEPI = () => {
    ...
    return (dispatch) => {
                            ...
                            const min = () => {
                                  ...
                                  if (minuteEPI >= 60) {
                                          minuteEPI = 60
                                          clearInterval(incrementEPI_BWH)
                                  } else {
                                          // foreground
                                          if (this.state.appState === 'active') {
                                                      if (secondEPI === 59) {
                            ...
                            clearInterval(incrementEPI_BWH)
                            incrementEPI_BWH = setInterval( () => dispatch({ 
...
const mapStateToProps = (state) => {  
  return {
    timerTextColorCPR: state.timerReducerCPR.timerTextColor_BWH,
    minuteCPR: state.timerReducerCPR.minute_BWH,
    secondCPR: state.timerReducerCPR.second_BWH,
    resetAllCPR: state.timerReducerCPR.resetAllCPR,

    timerTextColorEPI: state.timerReducerEPI.timerTextColor_BWH,
    minuteEPI: state.timerReducerEPI.minute_BWH,
    secondEPI: state.timerReducerEPI.second_BWH,
    resetAllEPI: state.timerReducerEPI.resetAllEPI_BWH,

    CPR: state.buttonsReducer.CPR_BWH,
    epi: state.buttonsReducer.epi_BWH,
    shock: state.buttonsReducer.shock_BWH,
  }
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    incrementCPR: () => dispatch({ type: 'INCREMENT_CPR_BWH' }),
    incrementEpi: () => dispatch({ type: 'INCREMENT_EPI_BWH' }),
    incrementShock: () => dispatch({ type: 'INCREMENT_SHOCK_BWH' }),
  }
}
