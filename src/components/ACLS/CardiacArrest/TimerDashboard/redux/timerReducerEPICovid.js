...
const initialState = {
  ...
  export default function timerReducerEPICovid (state = initialState, action) {
    if (action.type === 'START_TIMER_EPI_COVID') {
    ...
    } else if (action.type === 'START_TIMER_EPI_COVID_BWH') {
        if (action.resetAllEPI === true) {
          clearInterval(action.incrementType) 
          action.resetAllEPI = false
        }
